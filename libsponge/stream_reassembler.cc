...
void StreamReassembler::push_substring(const string& data, const size_t index,
     ...
     * _next_assembled_idx 这种可以截断前后部重复 data
     * 后直接装配，无需任何额外处理. 如果装不下，则插入 _unassemble_strs
     * 中并返回
     ...
     *      b. 若 up_idx + up_data.size() >
     * index，说明前半部分重合，将重合部分丢弃，并更新 index 之后获取 up_idx
     * 的下一个已经插入的 idx，下称 down_idx a. 若 index + data.size() <=
     * down_idx，则说明后半部分字串没有重合 b. 若 index + data.size() >
     * down_idx，说明后半部分重合 NOTE:
     * 需要注意的是，后半部分重合又有两种情况，一种是部分重合，另一种是与
     * down_data 完全重合。
     ...
     *
     *    NOTE:
     * 对于性能优化的考虑，可以考虑在丢弃重合部分后，合并至上一个或者下一个
     ...
     *
     * NOTE: 按照上述的操作，最终我们可以确保 _unassemble_strs
     * 中没有子串是相互重叠的
     *
     * 所以最终的实现情况是这样的：
     *  1. 获取当前 index 在 _unassemble_strs 中的上界 pair
     *      a. 如果存在上界
     * pair，则判断是否存在重复区域。如果存在部分重复则截断，存在全部重复则直接丢弃。
     *      b. 如果不存在上界 pair, 则判断当前数据是否已经和已经被 assemble
     * 的字符串重复。 如果存在部分重复则截断，存在全部重复则直接丢弃。
     *  2. 获取当前 index 在 _unassemble_strs 中的下界 pair。 如果存在下界 pair
     *      a. 如果下界 pair
     * 没有被当前数据完全包含，则判断是否存在重复区域。如果存在则截断。 b.
     * 如果下界 pair 被当前数据包含，则将下界 pair 从 _unassemble_strs
     * 中丢弃，之后重新取出一个新的下界 pair, 重复第二步 这时候我们可以获得与
     * _unassemble_strs 中没有任何重复的字符串
     *
     * NOTE:
     * 本人的整体操作是尽可能的降低内存消耗，用时间来换取空间，毕竟私以为数据比较珍贵，不能随意丢弃。
     */
     ...
     *  NOTE: 这里由于 unsigned 的结果向 signed
     * 的变量写入，因此可能存在上溢的可能
